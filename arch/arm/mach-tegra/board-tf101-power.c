...
static struct tps6586x_platform_data tps_platform = {
	...
	.subdevs = tps_devs,
	.gpio_base = TF101_TPS6586X_GPIO_BASE,
};
...
static int __init tf101_pcie_init(void)
{
	...
	ret = gpio_request(TF101_TPS6586X_GPIO_BASE, "pcie_vdd");
	if (ret < 0)
		goto fail;

	ret = gpio_direction_output(TF101_TPS6586X_GPIO_BASE, 1);
	if (ret < 0)
		goto fail;

	gpio_export(TF101_TPS6586X_GPIO_BASE, false);
	return 0;

 fail:
	pr_err("%s: gpio_request failed #%d\n", __func__, TF101_TPS6586X_GPIO_BASE);
	gpio_free(TF101_TPS6586X_GPIO_BASE);
	return ret;
}
