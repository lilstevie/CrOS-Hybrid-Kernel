...
static struct tegra_ulpi_config ulpi_phy_config = {
	.reset_gpio = TF101_GPIO_ULPI_RESET,
	.clk = "cdev2",
};
...
static void __init tf101_usb_init(void) {
...
static void __init tf101_touch_init(void) {
 ...
 * Power Supply
 *****************************************************************************/
static char *tf101_batteries[] = {
	"battery",
};

static struct resource tf101_power_resources[] = {
	[0] = {
		.name = "ac",
...
static int tf101_is_ac_online(void)
{
	return !gpio_get_value(TEGRA_GPIO_AC_ONLINE);
}

static void tf101_set_charge(int flags)
{
	gpio_direction_output(TEGRA_GPIO_VENTANA_DISABLE_CHARGER, !flags);
}

static int tf101_power_init(struct device *dev)
{
	int rc = 0;
...
static void tf101_power_exit(struct device *dev)
{
	gpio_free(TEGRA_GPIO_VENTANA_DISABLE_CHARGER);
	gpio_free(TEGRA_GPIO_AC_ONLINE);
}

static struct pda_power_pdata tf101_power_data = {
	.init = tf101_power_init,
	.is_ac_online = tf101_is_ac_online,
	.set_charge = tf101_set_charge,
	.exit = tf101_power_exit,
	.supplied_to = tf101_batteries,
	.num_supplicants = ARRAY_SIZE(tf101_batteries),
};

static struct platform_device tf101_powerdev = {
	.name = "pda-power",
	.id = -1,
	.resource = tf101_power_resources,
	.num_resources = ARRAY_SIZE(tf101_power_resources),
	.dev = {
		.platform_data = &tf101_power_data,
	},
};
...
static struct tegra_wm8903_platform_data tf101_audio_pdata = {
	.gpio_spkr_en		= TF101_GPIO_SPK_AMP,
	.gpio_hp_mute		= -1,
	.gpio_hp_det		= TF101_GPIO_HP_DETECT,
	.gpio_int_mic_en	= TF101_GPIO_MIC_EN_INT,
...
static struct wm8903_platform_data tf101_wm8903_pdata = {
	.micdet_delay = 100,
	.gpio_base = TF101_WM8903_GPIO_BASE,
	.gpio_cfg = {
...
static void __init tf101_sound_init(void) {
	i2c_register_board_info(0, &wm8903_device, 1);
	platform_device_register(&tf101_audio_device);
...
static void __init tf101_i2c_init(void)
{
...
static struct i2c_board_info __initdata tf101_i2c4_board_info[] = {
	{
...
static void __init tf101_sensors_init(void) {
...
static struct gpio_keys_button tf101_keys[] = {
	...
	{
		.code = KEY_VOLUMEDOWN,
		.gpio = TF101_GPIO_KEY_VOLUMEDOWN,
		.active_low = 1,
...
static void __init tf101_suspend_init(void) {
...
static void __init tegra_tf101_reserve(void)
{
